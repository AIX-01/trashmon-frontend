...
import { Sun, Cloud, ArrowLeft, Filter, ArrowDownUp } from 'lucide-react';
...
interface CharacterWithUrl {
  ...
  imageUrl: string;
  date: string;
  rawDate: Date; // 정렬을 위해 원본 날짜 객체 저장
  rank: string;
}
...
const FarmPage = () => {
  ...
  // 필터 및 정렬 상태
  const [filterCategory, setFilterCategory] = useState('전체'); // 기본값: 전체 (한글)
